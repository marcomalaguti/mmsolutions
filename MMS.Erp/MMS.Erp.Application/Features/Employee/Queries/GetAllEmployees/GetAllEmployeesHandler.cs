...
internal class GetAllEmployeesHandler(IEmployeeQueriesRepository employeeRepository,
                                      IMapper mapper) 
    : IQueryHandler<GetAllEmployeesQuery, IEnumerable<EmployeeDto>?>
{
    public async Task<Result<IEnumerable<EmployeeDto>?>> Handle(GetAllEmployeesQuery request,
